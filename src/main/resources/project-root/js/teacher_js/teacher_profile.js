document.addEventListener("DOMContentLoaded", () => {
  ...
  const addUserBirthdayInput = document.getElementById("addUserBirthday");
  const addUserParentsField = document.getElementById("parentsField");
  const addUserParentsInput = document.getElementById("addUserParents");
  const addUserClassField = document.getElementById("classField");
  const addUserClassInput = document.getElementById("addUserClass");
  const addUserSubjectField = document.getElementById("subjectField");
  const addUserSubjectInput = document.getElementById("addUserSubject");
  const addUserAdditionalInfoTextarea = document.getElementById("addUserAdditionalInfo");
  ...
  let otherUsers = [
    { id: 201, name: "Олена Петренко", email: "olena.petrenko@example.com", role: "Викладач", subject: "Математика", classTeacherOf: "11-Б", info: "Викладаю математику та є класним керівником 11-Б класу.", photo: "../assets/images/teacher-avatar.png" },
    { id: 202, name: "Андрій Сидоров", email: "andriy.sydorov@example.com", role: "Батько", parentOf: "Софія Сидорова", info: "Батько Софії Сидорової, учениці 5-Б класу.", photo: "../assets/images/parent-avatar.png" },
    { id: 203, name: "Софія Сидорова", email: "sofiya.sydorova@example.com", role: "Учень", birthday: "2010-11-05", class: "5-Б", parents: "Андрій та Наталія Сидорові", info: "Люблю малювати та читати.", photo: "../assets/images/student-avatar.png" }
  ];
  ...
  navToggle.addEventListener("click", () => sidebar.classList.toggle("collapsed"));
  ...
  function formatDate(dateStr) {
    ...
    return dateParts.length === 3 ? `${dateParts[2]}.${dateParts[1]}.${dateParts[0]}` : dateStr;
  }
  ...
  window.addEventListener("click", (e) => {
    ...
    if (e.target === profileViewModal) profileViewModal.style.display = "none";
  ...
  if (closeProfileViewModalBtn && profileViewModal) {
    closeProfileViewModalBtn.addEventListener("click", () => profileViewModal.style.display = "none");
  }

  if (searchUsersBtn && userSearchInput && userSearchResults) {
    searchUsersBtn.addEventListener("click", () => {
      const searchTerm = userSearchInput.value.trim().toLowerCase();
      const results = otherUsers.filter(user => user.name.toLowerCase().includes(searchTerm) || user.email.toLowerCase().includes(searchTerm));
      displaySearchResults(results);
    });
    userSearchInput.addEventListener("keypress", (event) => {
      if (event.key === "Enter") searchUsersBtn.click();
    });
  }
  ...
  function displaySearchResults(results) {
    ...
    if (results.length > 0) {
      results.forEach(user => {
        ...
        userItem.innerHTML = `<img src="${user.photo || '../assets/images/user-avatar-small.png'}" alt="Аватар" class="user-avatar-small"><span>${user.name}</span> (${user.email}) - ${user.role}`;
        userItem.addEventListener("click", () => displayOtherUserProfile(user));
  ...
  function updateAddUserFormFields(selectedRole) {
    const parentsField = document.getElementById("parentsField");
    ...
    if (selectedRole === "student") {
      ...
      parentsField.querySelector('label').textContent = 'Батьки:';
      ...
      classField.style.display = "none";
      subjectField.style.display = "none";
      parentsField.querySelector('label').textContent = 'Дитина:';
    } else {
      parentsField.style.display = "none";
      classField.style.display = "none";
      subjectField.style.display = "none";
  ...
  if (addUserRoleSelect) {
    addUserRoleSelect.addEventListener("change", (event) => {
      updateAddUserFormFields(event.target.value);
    });
  }
  ...
  if (openAddUserModalBtn && addUserModal) {
    openAddUserModalBtn.addEventListener("click", () => {
      ...
      updateAddUserFormFields(addUserRoleSelect.value);
    });
  }
  ...
  if (closeAddUserModalBtn && addUserModal) {
    closeAddUserModalBtn.addEventListener("click", () => {
      ...
      addUserForm.reset();
      updateAddUserFormFields(addUserRoleSelect.options[0].value);
    });
  }
  ...
  if (addUserForm && addUserModal) {
    addUserForm.addEventListener("submit", (e) => {
      ...
      const newMiddleName = addMiddleNameInput.value.trim();
      const newUserEmail = addUserEmailInput.value.trim();
      const newUserPassword = addUserPasswordInput.value;
      const newUserRole = addUserRoleSelect.value;
      const newUserBirthday = addUserBirthdayInput.value;
      const newUserChild = addUserParentsInput.value.trim();
      const newUserClass = addUserClassInput.value.trim();
      const newUserSubject = addUserSubjectInput.value.trim();
      const newUserAdditionalInfo = addUserAdditionalInfoTextarea.value.trim();

      console.log("Дані нового користувача:", {
        ...
        info: newUserAdditionalInfo
      });

      addUserModal.style.display = "none";
      addUserForm.reset();
      alert(`Користувача ${newFirstName} ${newLastName} додано! (Дані виведено в консоль)`);
    });
  }
});